...
// Probably should #[inline] all
pub fn counter(){
    unsafe{ externs::counter() }
}
pub fn print_u32(num: u32){
    unsafe{ externs::print_u32(num) }
}
pub fn rand() -> u64{
    unsafe{ externs::rand() }
}
pub fn hostmath(num1: u8, num2: i16, num3: u8) -> bool{
    ...
    unsafe{ externs::hostmath(num1, num2, num3) != 0 }
}
pub fn rand_2() -> (u32, bool){
    let val = unsafe{ externs::rand_2() };
    return (val.0, val.1 != 0);
}
pub fn print_buffer(buf: &[u8; 12]){
    unsafe{ externs::print_buffer(buf) }
}
pub fn print(string: &str){
    // Convert slice to FFI-compatible struct
    unsafe{ externs::print(string.as_bytes().into()) }
}
...
pub fn receive_big_buffer() -> Box::<[u8;104857600]>{
    unsafe{
        ...
        externs::receive_big_buffer(buf.as_mut());
        return buf;
    }
}
...
pub fn receive_string() -> String{
    unsafe{
        let len = externs::prepare_arbitrary_string();
        let mut dat: Vec<u8> = Vec::with_capacity(len);
        dat.set_len(len);
        let result = externs::receive_arbitrary(dat.as_mut_ptr());
        if(result != externs::ReceiveArbitraryResult::Sucess){ panic!("Problem with data received."); }
